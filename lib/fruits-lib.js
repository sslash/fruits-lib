	    ...
	    switch (action.type) {
	        ...
	        case types.VENUES_DIRECTIONS_MATRIX_FETCH_SUCCESS:
	            if (action.payload.data && action.payload.data.length) {
	                var newState = state.set('directionsMatrix', (0, _immutable.fromJS)({
	                    fetchingDirections: false,
	                    fetchingDirectionsError: false
	                }));
	
	                if (isNaN(action.index)) {
	                    return newState.setIn(['directionsMatrix', 'directionsResult'], (0, _immutable.fromJS)(action.payload));
	                } else {
	                    return newState.setIn(['directionsMatrix', 'directionsResult', 'data'], setDirectionResult(state.getIn(['directionsMatrix', 'directionsResult', 'data']), action.payload, action.index));
	                }
	            } else {
	                return state;
	            }
