...
/***/ function(module, exports, __webpack_require__) {
	...
	function fetchDistanceMatrixBetweenVenues(geoFirstSpot, geoSecondSpot, travelmode, routeId, index) {
	    if (checkGEO(geoFirstSpot) && checkGEO(geoSecondSpot)) {
	        var _ret = function () {
	            var lat = [geoFirstSpot.lat, geoSecondSpot.lat];
	            var lng = [geoFirstSpot.lng, geoSecondSpot.lng];
	            return {
	                v: {
	                    ...
	                    promise: function promise(_ref4) {
	                        var req = _ref4.req;
	                        return req.get('/routes/' + routeId + '/directionsMatrix', { params: { travelmode: travelmode, lat: lat, lng: lng } });
	                    },
	                    index: index
	                }
	            };
